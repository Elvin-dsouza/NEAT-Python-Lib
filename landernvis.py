...
def printGeneration(x):
	...
	for species in next_generation:
		print("species ", str(species.number), " Organisms: ",str(len(species.organisms)), "Improvement ",str(species.generationsSinceImrpovement))
		...
		for j,organism in enumerate(species.organisms):
			...
			for timestep in range(gen_timesteps):
				...
				if done:
					counter += 1
					break
			organism.global_fitness = score + 800
			# score = score+500
